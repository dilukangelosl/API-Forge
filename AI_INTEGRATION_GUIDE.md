...
6. **Developer Portal (Optional but Recommended)**: API Forge ships with a React SPA package (`@dilukangelo/api-forge-portal`) to manage applications and tokens. Mount its static files to your server (e.g., at `/portal`) with an SPA fallback, then access it in your browser (`http://localhost:3000/portal/`). **CRITICAL**: The static files for the portal MUST be mounted using `express.static()` **BEFORE** mounting any global `cors()` middlewares, otherwise the UI assets will throw 500 CORS errors.
    * **Note:** The portal frontend expects backend APIs at `/portal/api/*` (e.g., `/portal/api/apps`). You must manually implement these endpoints using the API Forge `Router`. Refer to the `examples/express-app/src/index.ts` file for a reference implementation.
