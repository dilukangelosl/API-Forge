...
export async function handleAuthorizationRequest(params: {
    ...
    const codeChallengeMethod = query.code_challenge_method as "S256" | "plain" | undefined;
    ...
    if (!client.redirectUris.includes(redirectUri)) {
        const error = OAuthErrors.invalidRequest(
    ...
    if (clientInvalidScopes.length > 0) {
        return errorRedirect(
            redirectUri,
            "invalid_scope",
            `Client is not authorized for scopes: ${clientInvalidScopes.join(", ")}`,
            state
        );
    }
    ...
    if (codeChallenge && codeChallengeMethod && !["S256", "plain"].includes(codeChallengeMethod)) {
        return errorRedirect(
            redirectUri,
            "invalid_request",
            "Invalid code_challenge_method. Supported: S256, plain",
            state
        );
    }
