...
export async function handleClientCredentialsGrant(params) {
    ...
    if (!client.isConfidential || !client.clientSecretHash) {
        const error = OAuthErrors.unauthorizedClient("Client credentials grant requires a confidential client");
    ...
    if (!client.grantTypes.includes("client_credentials")) {
        const error = OAuthErrors.unauthorizedClient("Client is not authorized for client_credentials grant");
        return Response.json(400, error.toJSON());
    }
    ...
    const { valid, filtered, invalid } = validateRequestedScopes(scopesToGrant, client.scopes);
    if (!valid) {
        const error = OAuthErrors.invalidScope(`Invalid scopes: ${invalid.join(", ")}`);
        return Response.json(400, error.toJSON());
    }
    // Generate tokens
 ...
 */
function extractClientCredentials(ctx) {
